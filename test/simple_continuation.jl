...
for alg in (Natural(), PALC(), PALC(tangent = Bordered()), Multiple(copy(x0), 0.01,13), PALC(tangent=Polynomial(Bordered(), 2, 6, copy(x0))), MoorePenrose())
    br0 = continuation(re_make(prob, params = opts.p_min), alg, opts; verbosity = 2)
    @test length(br0) > 10
    br0 = continuation(re_make(prob, params = opts.p_max), Natural(), ContinuationPar(opts, ds = -0.01); verbosity = 2)
    @test length(br0) > 10
end

# test with callbacks
br0 = continuation(prob, PALC(), (@set opts.max_steps = 3); callback_newton = (state; kwargs...)->(true), verbosity = 2);
...
br1 = continuation(prob, PALC(), opts; verbosity = 2) #(14.28 k allocations: 1001.500 KiB)
show(br1)
length(br1)
br1[1]
BK.eigenvals(br1,20)
...
br2 = continuation(prob, PALC(), opts; verbosity = 2)

# test for different norms
br3 = continuation(prob, PALC(), opts, normC = norminf, verbosity = 2)

# test for linesearch in Newton method
opts = @set opts.newton_options.linesearch = true
br4 = continuation(prob, PALC(), opts, normC = norminf, verbosity = 2) # (15.61 k allocations: 1.020 MiB)
...
br5 = continuation(prob, PALC(bls = BorderingBLS()), opts, normC = norminf, verbosity = 2)

br5 = continuation(prob, PALC(bls = MatrixBLS()), opts, normC = norminf, verbosity = 2)
...
br6 = continuation(prob, PALC(tangent = Secant()), opts, verbosity = 2)

optsnat = ContinuationPar(opts; ds = 0.001, dsmax = 0.1, dsmin = 0.0001)
br7 = continuation((@set prob.recordFromSolution = (x,p)->x[1]), Natural(), optsnat, verbosity = 2)

# tangent prediction with Bordered predictor
br8 = continuation(prob, PALC(tangent = Bordered()), opts, verbosity = 2)
...
# @set! opts9_1.newton_options.verbose = true
# @set! opts9_1.newton_options.max_iterations = 3
# br9_1 = continuation(prob,  Multiple(copy(x0), 1e-4,7), opts9_1, verbosity = 2)
...
br11 = continuation(prob, MoorePenrose(), opts11; verbosity = 2)
br11 = continuation(prob, MoorePenrose(method = BK.pInv), opts11; verbosity = 2)
br11 = continuation(prob, MoorePenrose(method = BK.iterative), opts11; verbosity = 2)
# plot(br11)
...
brsp = continuation(prob_sp, PALC(), opts, verbosity = 2)
brsp = continuation(prob_sp, PALC(bls = BK.BorderingBLS(solver = BK.DefaultLS(), check_precision = false)), opts, verbosity = 2)
brsp = continuation(prob_sp, PALC(bls = BK.MatrixBLS()), opts, verbosity = 2)
# plot(brsp,marker=:d)
####################################################################################################
# check bounds for all predictors / correctors
for talgo in (Bordered(), Secant())
    brbd  = continuation(prob, PALC(tangent = talgo), opts; verbosity = 2)
    @test length(brbd) > 2
end
prob.u0 .= ones(N)*3
@set! prob.params = -3.
...
@set! prob.params = -3.2
brbd  = continuation(prob, PALC(), ContinuationPar(opts, p_max = -2), verbosity = 2)
@test isnothing(brbd)
####################################################################################################
...
@set! prob.params = -1.5
br1 = continuation(prob, PALC(), ContinuationPar(opts; ds = -0.001), verbosity = 2)

br2 = continuation(prob, x0.u, -1.5, x1.u, -1.45, PALC(tangent = Bordered()), (@lens _), opts, verbosity = 2)
####################################################################################################
